...
# input_1 = 'tôi là một học sinh'

# input_text_2 = input_1
...
def main():
    tmp = translate.handle_input()
    trans_table = find_n_translation(tmp)
    # create the first population
    population = create_first_population(trans_table, tmp)
    best = []
    for i in range(100):
        print("Step:", i)
        new_population = []
        # evaluate the fitness of current population
        scores = scores_of_population(population, tmp)
        best = population[np.argmax(scores)]
        probability = fitness(best, tmp)               # chờ LM và TM
        print(best)
        print(probability)
        if probability > -10:
            break
